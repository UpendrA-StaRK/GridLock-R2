...
def step1_validate(project_root: Path, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    import pandas as pd
    df_raw = pd.read_csv(csv_path, nrows=5)  # read header first to check existence
    df_raw = pd.read_csv(csv_path, dtype=str)
    logger.info(f"Raw file loaded for validation: {len(df_raw):,} rows")

    report = validate_raw(df_raw, output_path=report_path)
...
def step2_ingest(project_root: Path, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    df, metadata = load_raw(csv_path, metadata_output_path=meta_path)
