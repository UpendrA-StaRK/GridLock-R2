...
def load_ranker(
    ...
    with tqdm(total=4, desc="Loading ranker", unit="step", leave=True) as pbar:
...
def _get_feature_cols(time_resolution: str, zone_grid_df: pd.DataFrame) -> list[str]:
    ...
    # Only include cols that actually exist in the grid
    return [c for c in candidates if c in zone_grid_df.columns]
...
def _build_zone_scaffold(
    ...
    # Compute median feature values per zone (stable representation)
    agg_dict: dict[str, str] = {}
    for col in feature_cols:
        if col not in ("hour_of_day", "is_weekend", "month", "zone_id"):
            agg_dict[col] = "median"

    if agg_dict:
        zone_medians = (
            zone_grid_df.groupby("zone_id")
            .agg(agg_dict)
            .reset_index()
        )
    else:
        zone_medians = zone_grid_df[["zone_id"]].drop_duplicates().reset_index(drop=True)

    scaffold = zone_medians.copy()
    ...
    # rolling_7d_count: use zone median from training data (best we can do at inference)
    # It will already be in zone_medians if computed by _get_feature_cols include check

    # Ensure correct column order and fill any missing with -1
    for col in feature_cols:
        if col not in scaffold.columns:
            scaffold[col] = -1
    scaffold = scaffold[feature_cols].fillna(-1)
