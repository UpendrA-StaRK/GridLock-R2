...
def _get_feature_cols(features_cfg: dict[str, Any], time_resolution: str) -> list[str]:
    ...
    For zone-grid training we use:
      ...
      spatial:     zone_id, fraction_at_junction (aggregated proxy for is_at_junction)
      historical:  rolling_7d_count (7-day lagged mean count per zone×hour — strongest signal)
      categorical: dominant_violation_type, dominant_vehicle_type, police_station_id,
                   center_code_encoded
    ...
    # Spatial
    cols += ["zone_id", "fraction_at_junction"]
...
def run_training(
    ...
    for time_resolution, model_name, model in tqdm(
        ...
        _save_checkpoint(ckpt_dir, model, model_name, eval_result, configs, project_root)
