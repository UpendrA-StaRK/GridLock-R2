...
# Find the best lightgbm_hour checkpoint (most recent by timestamp)
ckpt_dir_base = PROJECT_ROOT / "checkpoints"
lgb_hour_dirs = sorted(
    [d for d in ckpt_dir_base.glob("lightgbm_hour_*") if d.is_dir()],
    key=lambda d: d.name,
    reverse=True,
)
if not lgb_hour_dirs:
    raise FileNotFoundError(
        "No lightgbm_hour_* checkpoint found. Run training (Phase 1) first."
    )

best_ckpt = lgb_hour_dirs[0]
logger.info(f"Using checkpoint: {best_ckpt.name}")

# Load LightGBM model
import lightgbm as lgb
model = lgb.Booster(model_file=str(best_ckpt / "model.lgb"))
logger.info(f"LightGBM model loaded | n_trees={model.num_trees()}")
...
# Add cyclical temporal features if missing
for df_split in [train_df, test_df]:
    if "hour_sin" not in df_split.columns and "hour_of_day" in df_split.columns:
        df_split["hour_sin"] = np.sin(2 * np.pi * df_split["hour_of_day"] / 24.0)
        df_split["hour_cos"] = np.cos(2 * np.pi * df_split["hour_of_day"] / 24.0)
    if "dow_sin" not in df_split.columns and "day_of_week" in df_split.columns:
        df_split["dow_sin"] = np.sin(2 * np.pi * df_split["day_of_week"] / 7.0)
        df_split["dow_cos"] = np.cos(2 * np.pi * df_split["day_of_week"] / 7.0)
...
# For LightGBM regression, shap_values is 2D; extract if needed
...
plt.title("SHAP Feature Importance — GridLock R2 LightGBM (Zone Enforcement Priority)", fontsize=13)
plt.tight_layout()
